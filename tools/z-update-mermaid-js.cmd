...
set "SCRAPED_URLROOT=https://nightly.link/mermaid-js/mermaid/actions/runs/5007025176"
...
set "FILENAME=mermaid-build.zip"
set "URL=%SCRAPED_URLROOT%/%FILENAME%"
set "dstfile=%FILENAME%"
set "srcfile=%URL%"
echo Updating %FILENAME% ...
if "xy" == "xy" if not exist "%dstfile%" call curl -o "%dstfile%" "%srcfile%" -L -H "%Referer%" -H "%UserAgent%" -H "DNT: 1" --compressed
set "dstfile=%DESTDIR%/mermaid.min.js"
if not exist "%dstfile%" call 7z e %FILENAME% mermaid.min.js -o%DESTDIR%
